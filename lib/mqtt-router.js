...
/*
 * emq-router
 * https://github.com/tothjay/emq-router
 *
...
var Router = function(mqttclient) {
  ...
  this.subscribe = function(topic, opts, handler) {
    ...
    var topicRoute = topic;
    
    // clean out the EMQTT special wild card options
    if (topic.indexOf('$share') > -1) {
      var emqttWildCard = `$share/${topic.split('/')[1]}/`;
      topicRoute = topic.replace(emqttWildCard, '');
      
    } else if (topic.indexOf('$queue') > -1) {
      topicRoute = topic.replace('$queue', '');
    }
    // clean out the MQTT wild card options
    var routeOption = topicRoute.replace(/\$/, "\\$").replace(/\+/, "").replace(/#/, "");
